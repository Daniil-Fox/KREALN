...
// SpBlockHero
const spHero = document.querySelector('.sp-block-hero__img');
if (spHero) {
...
if (contactsInitCont && contactsInitCont.length > 0) {
  ...
  function initOrbit(container) {
    ...
    function disableBtn() {
      if (currentIndex == orbitActive.length - 1) nextBtn.classList.add('disable');else nextBtn.classList.remove('disable');
      if (currentIndex == 0) prevBtn.classList.add('disable');else prevBtn.classList.remove('disable');
    }
...
if (window.matchMedia('(max-width: 1024px)').matches && blogItems && blogItems.length > 0) {
  ...
  blogItems.slice(visible).forEach(el => el.classList.add('hide'));
  showMore?.addEventListener('click', e => {
    visible += 3;
    e.preventDefault();
    blogItems.slice(0, visible).forEach(el => el.classList.remove('hide'));
...
/***/ "./src/js/components/contacts-tab.js":
/*!*******************************************!*\
  !*** ./src/js/components/contacts-tab.js ***!
  \*******************************************/
/***/ ((__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) => {
...
function handleLongSlideScroll(slide, delta) {
  ...
  if (delta > 0) {
    if (currentScroll < maxScroll) {
      slide.scrollTop = Math.min(currentScroll + delta, maxScroll);
...
const researchSlider = new swiper__WEBPACK_IMPORTED_MODULE_0__.Swiper('.research__slider', {
  slidesPerView: 'auto',
  centeredSlides: true,
  loop: true,
...
new swiper__WEBPACK_IMPORTED_MODULE_0__.Swiper('.team-people__slider', {
  slidesPerView: 'auto',
  spaceBetween: 0,
  // centeredSlides: true,
  freeMode: true,
...
const toggleSwiper = (container, swiperClass, swiperSettings, tabItem, callback) => {
  let swiper;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const someFunc = instance => {
    if (instance) {
      instance.on('slideChangeTransitionEnd', function (e) {
        instance.el.style.height = instance.el.querySelector('.swiper-slide-active').scrollHeight + 'px';
      });
    }
  };
  resizableSwiper('(max-width: 1024px)', '.completed__slider', {
    slidesPerView: 1,
    spaceBetween: 12,
    // slidesPerGroup: 2,
    //   grid: {
  ...
  resizableSwiper('(min-width: 1025px)', '.blog-slider__slider', {
    slidesPerView: 1,
    spaceBetween: 40,
    // mousewheel: true,
    speed: 500,
